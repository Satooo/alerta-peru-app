...
export function participacion(incidentes,user_id){
                    ...
                    {Array(incidentes.length).fill(0).map((_,index)=>{
                        if(incidentes.length>0){
                            if(incidentes[index].user_id==user_id){
                                return (
                                    incidenteCard(incidentes[index].titulo,incidentes[index].descripcion, incidentes[index].fecha,incidentes[index].tipo,incidentes[index].validacion_status, incidentes[index].descripcionCompleta,incidentes[index].user,incidentes[index].incidente_id)
                                )
                            }
                        }
                    })}
